...
class Trackings implements TrackingsInterface {
    ...
    public function createTracking($params = [])
    {
        ...
        if (empty($params['courier_code'])) {
            throw new Tracking51Exception('Courier Code cannot be empty');
        }
        $this->apiPath = 'create';
    ...
    {
        if (count($params)>40) {
            throw new Tracking51Exception('Max. 40 tracking numbers create in one call');
        }
    ...
    {
        if (empty($idString)) {
            throw new Tracking51Exception('Id cannot be empty');
        }
