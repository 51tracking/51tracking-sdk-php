...
use Tracking51\Interfaces\TrackingsInterface;
...
class Trackings implements TrackingsInterface {
    ...
    public function createTracking($params = [])
    {
        ...
        if (empty($params['courier_code'])) {
            throw new Tracking51Exception(ErrorMessages::ErrMissingCourierCode);
        }
        $this->apiPath = 'create';
    ...
    {
        if (count($params)>40) {
            throw new Tracking51Exception(ErrorMessages::ErrMaxTrackingNumbersExceeded);
        }
    ...
    {
        if (empty($idString)) {
            throw new Tracking51Exception(ErrorMessages::ErrEmptyId);
        }
